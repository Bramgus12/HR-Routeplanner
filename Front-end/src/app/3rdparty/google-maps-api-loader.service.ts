...
export class GoogleMapsApiLoaderService {
  ...
  loadAPI(config: MapsApiConfig){
    return new Promise((resolve, reject) => {

      let script = document.createElement('script');
      // script.src = 'https://maps.googleapis.com/maps/api/js?key=YOUR_API_KEY&callback=initGoogleMaps';
      script.src = `https://maps.googleapis.com/maps/api/js?${this.getParams(config)}&callback=initGoogleMaps`;
      script.defer = true;
      script.async = true;
  ...
  getParams(config: MapsApiConfig){
